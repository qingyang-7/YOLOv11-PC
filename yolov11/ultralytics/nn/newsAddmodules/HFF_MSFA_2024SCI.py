...
class HFF_block(nn.Module):
    def __init__(self, in_ch, r_2=2, drop_rate=0.1):
        super(HFF_block, self).__init__()
        ch_1, ch_2, ch_int, ch_out =in_ch,in_ch,in_ch,in_ch
    ...
    def forward(self, data):
        if len(data)==2:
            l, g, f = data[0], data[1],None
        else:
            l, g, f = data[0], data[1], data[2]
...
if __name__ == "__main__":
    hff_block = HFF_block(128)
    l = torch.randn(1, 128, 64, 64)  # 本地特征
    g = torch.randn(1, 128, 64, 64)  # 全局特征
    f = torch.randn(1, 128, 2*64, 2*64)  # 额外特征
    # 运行 HFF_block
    output = hff_block([l, g])
    # 打印输出信息
