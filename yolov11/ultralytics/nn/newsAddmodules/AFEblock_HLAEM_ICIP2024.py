...
__all__=['C2f_AFEblock','C3k2_AFEblock','C2f_HLAEM','C3k2_HLAEM','AFEBlock','HLAEM']
# 论文地址;https://arxiv.org/abs/2407.09379
'''
来自 ICIP2024 
即插即用模块： AFEBlock 自适应特征增强模块
 含二次创新模块 HLAEM 高低频特征自适应增强模块
...
class C2f_AFEblock(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(AFEBlock(self.c) for _ in range(n))
...
class C3k(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        self.m = nn.Sequential(*(AFEBlock(c_) for _ in range(n)))

class C3k2_AFEblock(C2f_AFEblock):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else AFEBlock(self.c) for _ in range(n)
        )
...
class C2f_HLAEM(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(HLAEM(self.c) for _ in range(n))
...
class C3k_HLAEM(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(HLAEM(c_) for _ in range(n)))

class C3k2_HLAEM(C2f_HLAEM):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            C3k_HLAEM(self.c, self.c, 2, shortcut, g) if c3k else HLAEM(self.c) for _ in range(n)
        )
...
最终提升低频特征的精细度。
'''
class HLAEM(nn.Module): #高低频特征自增强模块
