            ...
            if self.comp_feat_upsample:
                if self.use_high_pass:
                    ...
                    mask_lr_lr_feat = F.interpolate(
                        carafe(mask_lr_lr_feat_lr, mask_lr_init.to(compressed_hr_feat.dtype), self.lowpass_kernel,
                    ...
                    mask_hr_lr_feat = F.interpolate(
                        carafe(self.content_encoder2(compressed_lr_feat), mask_lr_init.to(compressed_hr_feat.dtype),
                               self.lowpass_kernel, self.up_group, 2), size=compressed_hr_feat.shape[-2:],
                        mode='nearest')
...
if __name__ == "__main__":
    ...
    # ).cuda()
    freq_fusion = FreqFusion([64,64]).cuda()
