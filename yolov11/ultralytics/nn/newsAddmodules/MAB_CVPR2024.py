...
# 代码;https://github.com/icandle/MAN/blob/main/archs/MAN_arch.py
# 论文：https://arxiv.org/abs/2209.14145
__all__=['MAB','C3k2_MAB','C2f_MAB']
'''
                         CVPR 2024顶会论文
即插即用模块：MAB 多尺度注意力模块 、MLKA多尺度大核注意力、GSAU门控空间注意单元

在该论文中，多尺度注意块（Multi-scale Attention Block, MAB）包含两个主要模块：
多尺度大核注意力（Multi-scale Large Kernel Attention, MLKA）模块和门控空间注意单元（Gated Spatial Attention Unit, GSAU）。
...
适用于：图像分割，目标检测，图像增强，图像恢复，暗光增强,超分辨率图像任务等所有计算机视觉CV任务通用模块
'''
...
class MAB(nn.Module):
    def __init__(self, n_feats):
        super().__init__()
        self.c1 = (n_feats // 3 ) * 3
        self.LKA = MLKA(self.c1)
        self.LFE = GSAU(self.c1)
        self.conv1 = nn.Conv2d(n_feats, self.c1,1, 1)
        self.conv2 = nn.Conv2d(self.c1,n_feats,1,1)
    def forward(self, x):
