...
__all__ = ['FFTformer','FSASDFFN','FSAS','C3k2_FSASDFFN','C2f_FSASDFFN']
...
本文提出了一种新的频域自注意力机制(FSAS)和频域前馈网络设计(DFFN)，来解决传统图像去模糊模型中的计算效率和性能问题。

主要贡献：   
频域自注意力模块(FSAS)：通过在频域进行点乘操作来替代空间域中的矩阵乘法，减少了空间和时间复杂度。
频域前馈网络 (DFFN)：借鉴JPEG压缩算法，通过门控机制有选择性地保留特征中的低频和高频信息，提高清晰图像的重建效果。
非对称编码器-解码器网络：只在解码器模块中使用FSAS，以避免浅层特征中的模糊效应影响全局内容的提取，从而更好地恢复清晰图像。

FSAS模块原理及作用： FSAS利用空间域的卷积等价于频域的逐点乘法，
因此通过在频域进行自注意力的计算，显著降低了Transformer中原有的复杂度。
这不仅减少了空间和计算时间，还有效提升了高分辨率图像去模糊的处理性能。
...
class C2f(nn.Module):
    ...
    def forward_split(self, x):
        ...
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, 1))
class C2f_FSASDFFN(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(FSASDFFN(self.c) for _ in range(n))
class C3k(C3):
...
class C3k2_FSASDFFN(C2f_FSASDFFN):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
            """Initializes the C3k2 module, a faster CSP Bottleneck with 2 convolutions and optional C3k blocks."""
            super().__init__(c1, c2, n, shortcut, g, e)
            self.m = nn.ModuleList(
                C3k(self.c, self.c, 2, shortcut, g) if c3k else  FSASDFFN(self.c) for _ in range(n)
            )
