...
from win32con import TAPE_DRIVE_ERASE_SHORT
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask],            # 当使用SDIoU损失函数时，令d=0.3,0.5,0.7都可以
                       xywh=False, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False,SDIoU=False,d=0.3,
                       WIoU=False, ShapeIoU=True, Inner=False,  mpdiou=False,  ratio=0.75, eps=1e-7, scale=0.0)
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        self.assigner = TaskAlignedAssigner(topk=tal_topk, num_classes=self.nc, alpha=0.5, beta=6.0)
        self.bbox_loss = BboxLoss(m.reg_max, nwd_loss=self.nwdloss,iou_ratio=self.iou_ratio).to(device)
